...
# Gated Attention Residual U-Net
class GatedAttentionUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Decoder with attention-based skip connections
        x = self.up4(x)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=50):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_val_iou > best_val_iou:
            ...
            try:
                torch.save(model.state_dict(), "best_gatedunet_model.pth")
...
def main():
    ...
    # Create the model
    model = GatedAttentionUNet(in_channels=3, out_channels=13).to(device)
    ...
    try:
        model.load_state_dict(torch.load("best_gatedunet_model.pth"))
