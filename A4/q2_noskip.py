...
class UNetNoSkip(nn.Module):
    def __init__(self, in_channels=3, out_channels=13):
        ...
        # Decoder (no skip connections, so reduced input channels)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=50):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_val_iou > best_val_iou:
            ...
            try:
                torch.save(model.state_dict(), "best_noskipnet_model.pth")
...
def main():
    ...
    # Create the model
    model = UNetNoSkip(in_channels=3, out_channels=13).to(device)
    ...
    try:
        model.load_state_dict(torch.load("best_noskipnet_model.pth"))
